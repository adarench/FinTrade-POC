import { Trade, UserPortfolio, PortfolioHolding, CopySettings } from '../types';
import { getStockPrice } from './yahooFinance';

// In-memory storage of user portfolios for demo
const userPortfolios = new Map<number, UserPortfolio>();

/**
 * Initialize a new user portfolio
 */
export function initializeUserPortfolio(userId: number, initialBalance: number = 100000): UserPortfolio {
 ...
 */
export function getUserPortfolio(userId: number): UserPortfolio | undefined {
 ...
 */
export function followTrader(userId: number, traderId: number): boolean {
 ...
 */
export function unfollowTrader(userId: number, traderId: number): boolean {
  const portfolio = userPortfolios.get(userId);
  if (!portfolio) return false;
 ...
 */
export function updateCopySettings(userId: number, traderId: number, settings: CopySettings): boolean {
  const portfolio = userPortfolios.get(userId);
  if (!portfolio) return false;
 ...
 */
export async function processTrade(trade: Trade): Promise<{userId: number, copyTrade: Trade}[]> {
  const results: {userId: number, copyTrade: Trade}[] = [];
  
  // Check all user portfolios
  for (const [userId, portfolio] of userPortfolios.entries()) {
    // Skip if not following this trader
    if (!portfolio.traders_followed.includes(trade.trader_id)) continue;
 ...
 */
export async function executeTrade(userId: number, trade: Trade): Promise<boolean> {
  const portfolio = userPortfolios.get(userId);
  if (!portfolio) return false;
  ...
  try {
    currentPrice = await getStockPrice(trade.symbol);
  } catch (error) {
 ...
 */
function updateAllocations(portfolio: UserPortfolio): void {
 ...
 */
function calculateTotalPortfolioValue(portfolio: UserPortfolio): number {
 ...
 */
export async function updatePortfolioPrices(userId: number): Promise<boolean> {
  const portfolio = userPortfolios.get(userId);
  if (!portfolio) return false;
